...
export async function readProductBody(request: Request) {
  ...
  if (contentType.includes("multipart/form-data")) {
    let form: FormData;
    try { form = await request.formData(); } catch { throw new ApiError(400, "No se pudo procesar el formulario de producto."); }
    const body: Record<string, unknown> = {};
    for (const field of productFields) {
      const value = form.get(field);
      if (typeof value === "string" && value.length > 0) body[field] = ["featured"].includes(field) ? parseBoolean(value) : value;
    }
  ...
  try { return await request.json(); } catch { throw new ApiError(400, "El cuerpo de la petición debe ser JSON válido."); }
}