...
export async function POST(request: Request) {
  try {
    const user = await requireRequestAdmin(); getDatabaseUrl(); const parsed = productSchema.safeParse(await readProductBody(request)); if (!parsed.success) throw new ApiError(422, parsed.error.issues[0]?.message ?? "Datos inválidos.", parsed.error.issues.map((issue) => ({ field: issue.path.join(".") || "(root)", message: issue.message })));
    const sku = parsed.data.sku || `SKU-${Date.now()}-${Math.random().toString(36).substring(2, 6).toUpperCase()}`;
    const product = await getPrisma().$transaction(async (transaction) => { const category = await transaction.category.findUnique({ where: { id: parsed.data.categoryId } }); if (!category) throw new ApiError(422, "La categoría seleccionada no existe."); const created = await transaction.product.create({ data: { ...parsed.data, sku }, include: { category: true } }); if (created.stock > 0) await transaction.inventoryMovement.create({ data: { productId: created.id, userId: user.id, type: "ENTRY", quantity: created.stock, reason: "Stock inicial" } }); return created; });
